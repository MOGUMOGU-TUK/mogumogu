 ...
 */
export function useChatMessages(gongguId: string, authenticated = false): ChatMessage[] | null {
  const [messages, setMessages] = useState<ChatMessage[] | null>(() =>
    isFirebaseConfigured() ? [] : null
  );

  useEffect(() => {
    if (!isFirebaseConfigured() || !authenticated || !gongguId) {
      setMessages(isFirebaseConfigured() ? [] : null);
      return;
    }

    return subscribeMessages(gongguId, setMessages);
  }, [gongguId, authenticated]);
